...
} from "@eb/shared/src/graphql/apollo-types/ExperienceConnectionFragment";
...
export function floatExperienceToTheTopInGetExperiencesMiniQuery(
  ...
  let experienceEdge =
    undefined as unknown as ExperienceConnectionFragment_edges;
...
export function floatExperiencesToTopInGetExperiencesMiniQuery(ids: {
  ...
  const updatedExperienceConnection = immer(experienceConnection, (proxy) => {
    ...
    proxy.edges = edges.sort((a, b) => {
      const id1 = (
        (a as ExperienceConnectionFragment_edges)
          .node as ExperienceConnectionFragment_edges_node
      ).id;

      const id2 = (
        (b as ExperienceConnectionFragment_edges)
          .node as ExperienceConnectionFragment_edges_node
      ).id;
...
export type PurgeExperiencesFromCache1Fn = {
  purgeExperiencesFromCache1Fn: typeof purgeExperiencesFromCache1;
};
